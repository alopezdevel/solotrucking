...
function inicio(){  
        ...
        $('#eTipoLicencia').change(function(){get_states($(this).val());});  
...
function driver_save(){
    ...
    if ( valid ) { 
       ...
       if($('#drivers_edit #iConsecutivo').val() != ''){struct_data_post.edit_mode = "true";}else{struct_data_post.edit_mode = "false";}
       struct_data_post.action="save_driver";
       struct_data_post.domroot= "#drivers_edit"; 
        $.post("funciones_drivers.php",struct_data_post.parse(),
        function(data){
            switch(data.error){
             case '0':
                alert(data.msj);
...
function driver_edit(id){
    ...
    if(id != ""){
        ...
        {accion: "load_driver", id_driver : id, domroot : 'drivers_edit'},
           function(data){ 
               switch(data.error){
                 case '0':
                    eval(data.fields);
...
function filter_data(){
    ...
    if($("#flt_LicenseType").val() != ""){ filters += "eTipoLicencia|"+$("#flt_LicenseType").val()+","}   
    ...
    <div id="ct_drivers" class="container">
        ...
        <thead>
            <tr id="grid-head1">
                ...
                <td><input id="flt_expirationdate" type="date" placeholder="Expiration Date:"></td> 
                <td>
                    <select id="flt_LicenseType">
            ...
            <tr id="grid-head2">
                ...
                <td class="etiqueta_grid">Date of Birth</td>
                <td class="etiqueta_grid">License Expiration Date</td>
    ...
    <div class="p-container">
        <form>
            <fieldset>
                ...
                <div> 
                    <label>License Number: </label><input id="iNumLicencia" class="only_numbers" maxlength="10" type="text" placeholder="Please write the license number...">
                </div>
                <div> 
                    <label>Licence Expiration Date: </label><input id="dFechaExpiracionLicencia" type="date" placeholder="">
                </div>
            </fieldset>
            <fieldset>
