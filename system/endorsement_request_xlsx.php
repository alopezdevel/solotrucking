<?php
  ...
  #PARAMETERS:
  $flt_tipo     = trim($_GET['reporte_tipo']); 
  $error        = 0;
  $fecha_inicio = urldecode($_GET['flt_dateFrom']);
  $fecha_fin    = urldecode($_GET['flt_dateTo']);
  ...
  //Otros filtros:
  $flt_company      = urldecode($_GET['reporte_company']);
  $flt_policy       = urldecode($_GET['reporte_policy']);
  $flt_name         = urldecode($_GET['name']);
  $flt_broker       = urldecode($_GET['reporte_broker']);
  $nombre_parametro = "";
  ...
  if($rows == 0){ $error = 1; $mensaje = "No results found.";}
  else{ 
      ...
      #----------------------------------------------- EXCEL BEGINS ---------------------------------------------------------------#
        ...
        $objPHPExcel->getProperties()->setCreator("Solo-Trucking Insurance System")
        ->setLastModifiedBy("Solo-Trucking Insurance System")->setTitle("Solo-Trucking Insurance On-line Reports")
        ->setSubject("endorsements")
        ->setDescription("Report of endorsements registered in the system")
        ->setKeywords("office 2007 openxml php")->setCategory("result file");
        ...
        $objPHPExcel->getActiveSheet()->setSharedStyle($EstiloEncabezado2, "A2:J2");
        ...
        ->setCellValue('D'.$row, 'DOB')
        ->setCellValue('E'.$row, 'LICENSE NUMBER')
        ->setCellValue('F'.$row, 'LICENSE TYPE')
        ->setCellValue('G'.$row, 'EXPIRE DATE')
        ->setCellValue('H'.$row, 'EXPERIENCE YEARS')
        ->setCellValue('I'.$row, 'APP DATE')
        ->setCellValue('J'.$row, 'POLICIES'); 
        ...
        while ($endoso = $result->fetch_assoc()) {
            ...
            if($r->num_rows > 0){
                ...
                /*while ($detalle = $r->fetch_assoc()) {
                    ...
                    ->setCellValue('B'.$row, $detalle['eAccion'])
                    ->setCellValue('C'.$row, $detalle['sNombre'])
                    ->setCellValue('D'.$row, $detalle['DOB'])
                    ->setCellValue('E'.$row, $detalle['iNumLicencia'])
                    ->setCellValue('F'.$row, $detalle['eTipoLicencia'])
                    ->setCellValue('G'.$row, $detalle['dFechaExpiracion'])
                    ->setCellValue('H'.$row, $detalle['iExperienciaYear'])
                    ->setCellValue('I'.$row, $endoso['dFechaAplicacion'])
                    ->setCellValue('J'.$row, $polizas);
                ...
                #CREATE A FILE NAME:
                $nombre_archivo = "Report_of_endorsements_from";//.$fecha_inicio." to ".$fecha_fin;
