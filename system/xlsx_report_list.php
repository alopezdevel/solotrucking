<?php
    ...
    */
    ...
    ->setDescription("Report of all list of drivers and/or vehicles of a company.")
    ...
    if($reporttype == "" || $reporttype == 'all' || $reporttype == '2'){
         if($policy == 'all'){
             $query  = "SELECT iConsecutivo, sNombre, DATE_FORMAT(dFechaNacimiento,'%m/%d/%Y') AS dFechaNacimiento, DATE_FORMAT(dFechaExpiracionLicencia,'%m/%d/%Y') AS dFechaExpiracionLicencia, ".
                       ...
                       "FROM ct_operadores ".$filtro_query.$orden_query;
         }else{
         ...
         if($rows > 0){
             ...
             $descripcionReporte = 'Driver list from '.strtoupper($DatosCo['sNombreCompania']);
             ...
             $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('H'.$row, 'IS IN POLICIES');   

             while ($items = $result->fetch_assoc()){
                 $row++;
                 $objPHPExcel->getActiveSheet()->setSharedStyle($EstiloContenido, "A".$row.":H".$row); 
                 ...
                 //Reporte contenido:
                 $objPHPExcel->setActiveSheetIndex(0)
                             ->setCellValue('A'.$row, $items['sNombre'])
                             ->setCellValue('B'.$row, $items['dFechaNacimiento'])
                             ->setCellValue('C'.$row, $items['iNumLicencia'])
                             ->setCellValue('D'.$row, $items['TipoLicencia'])
                             ->setCellValue('E'.$row, $items['dFechaExpiracionLicencia'])
                             ->setCellValue('F'.$row, $items['iExperienciaYear'])
                             ->setCellValue('G'.$row, $textoIngreso)
                             ->setCellValue('H'.$row, $polizas); 
    ...
    #CREATE A FILE NAME:
    $info_fecha = getdate();
    $nombre_archivo = 'Report of list'.strtoupper($DatosCo['sNombreCompania'])." - ".$info_fecha['year']."_".$info_fecha['month']."_".$info_fecha['mday'];
