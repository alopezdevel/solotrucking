<?php
  ...
  function get_endorsements(){
    ...
    $query_rows = "SELECT COUNT(A.iConsecutivo) AS total FROM cb_endoso_adicional AS A ".
                  "LEFT JOIN cb_endoso_adicional_detalle AS B ON A.iConsecutivo = B.iConsecutivoEndoso ".
    ...
    if($registros == "0"){
      ...
      $sql    = "SELECT A.iConsecutivo, D.sNombreCompania,A.eStatus, DATE_FORMAT(A.dFechaAplicacion, '%m/%d/%Y %H:%i') AS dFechaIngreso, D.iOnRedList, B.sNombreCompania AS sNombreAdicional, B.sDireccion, B.sEstado, B.sCiudad, B.sCodigoPostal, B.eTipoEndoso, B.eAccion ".
      ...
      if($rows > 0){    
            while ($item = $result->fetch_assoc()){ 
                 ...
                 $item["eAccion"] == "ADDSWAP"    ? $action = "ADD SWAP"       : "";
                 $item["eAccion"] == "DELETESWAP" ? $action = "DELETE SWAP"    : "";
                 $action          == ""           ? $action = $item["eAccion"] : "";
                 
                 $sDescripcion = $item['sNombreAdicional'].'<br><span style="font-size:11px!important;">'.$item['sDireccion'].', '.$item['sCiudad'].', '.$item['sEstado'].', '.$item['sCodigoPostal'].'</span>';
  ...
  function save_endorsement(){
      ...
      if($edit_mode == 'true'){
            ...
            foreach($_POST as $campo => $valor){
                if($campo != "accion" && $campo != "edit_mode" && $campo != "iConsecutivo" && !(strpos($campo,"chk_policies_") && $campo != "dFechaAplicacionHora")){ // Estos campos no se insertan a la tabla
      ...
      if($iConsecutivo != ""){
          ...
          if($rows['total'] > 0){
            ...
            while ($items = $result->fetch_assoc()) { 
                 ...
                 $htmlTabla .= "<tr>".
                               "<td id=\"idUnit_".$items['iConsecutivoDetalle']."\">".$action."</td>".
