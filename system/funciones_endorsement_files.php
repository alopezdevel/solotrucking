<?php
  ...
  function get_endorsements(){
    ...
    $query_rows = "SELECT COUNT(A.iConsecutivo) AS total ".
                  ...
                  "LEFT JOIN ct_unidades D ON A.iConsecutivoUnidad = D.iConsecutivo ".
                  "LEFT JOIN ct_companias E ON A.iConsecutivoCompania = E.iConsecutivo ".$filtroQuery;
    ...
    if($registros == "0"){
      ...
      $sql = "SELECT A.iConsecutivo, B.sDescripcion AS categoria, A.eStatus, eAccion, sNombre, sVIN, sNombreCompania,iOnRedList, iConsecutivoTipoEndoso ".
             "FROM cb_endoso A ".
             "LEFT JOIN ct_tipo_endoso B ON A.iConsecutivoTipoEndoso = B.iConsecutivo ".
             "LEFT JOIN ct_operadores C ON A.iConsecutivoOperador = C.iConsecutivo ".
             "LEFT JOIN ct_unidades D ON A.iConsecutivoUnidad = D.iConsecutivo ".
             "LEFT JOIN ct_companias E ON A.iConsecutivoCompania = E.iConsecutivo ".$filtroQuery.$ordenQuery." LIMIT ".$limite_inferior.",".$limite_superior;
        ...
        if ($rows > 0) {    
            while ($items = $result->fetch_assoc()) { 
               if($items["iConsecutivo"] != ""){
                     ...
                     if(strtoupper($items['categoria']) == 'DRIVER'){
                         $descripcion = strtoupper($items['sNombre']);
                     }else if(strtoupper($items['categoria']) == 'UNIT OR TRAILER'){
                         $descripcion = strtoupper($items['sVIN']);
                     }
                     $htmlTabla .= "<tr $class>
                                        <td>".$items['iConsecutivo']."</td>".
                                       "<td>".$redlist_icon.$items['sNombreCompania']."</td>".
                                       "<td>".$descripcion."</td>". 
                                       "<td class=\"".$items['iConsecutivoTipoEndoso']."\">".strtoupper($items['categoria'])."</td>".                                                                                                                                                                                                                       
                                       "<td>".
