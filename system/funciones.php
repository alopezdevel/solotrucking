<?php 
  ...
  function  conexion(){                
    ...
    $queryUsuario = "SELECT iConsecutivoTipoUsuario 
                     FROM cu_control_acceso WHERE sCorreo = '".$usuario."'  AND hActivado in ( '1','2') ";    
    ...
    //validando query de acceso
    $queryUsuarioAcceso = "SELECT iConsecutivoTipoUsuario 
                           FROM cu_control_acceso 
                           WHERE sCorreo = '".$usuario."' AND hClave = sha1('".$clave."') AND hActivado in ( '1','2') ";    
    ...
    if ( ($usuario == "masterusersystem" && $clave == "mastersolotrucking") || $NUM_ROWs_Usuario_Acceso == 1){
        ...
        $acceso = $UsuarioAcceso['iConsecutivoTipoUsuario'];
...
#FUNCIONES PARA USUARIOS Y CERTIFICADOS:
  ...
  function get_company(){   
    ...
    $sql = "SELECT  C.hContenidoDocumentoDigitalizadoAdd, A.iConsecutivo as id, sNombreCompania, eEstatusCertificadoUpload as estatus_upload, sUsdot,
            ...
            FROM  ct_companias A 
            LEFT JOIN cb_certificate_file C ON A.iConsecutivo = C.iConsecutivoCompania ORDER BY A.iConsecutivo ASC";
    $result = $conexion->query($sql);
           ...
           if($usuario["sNombreCompania"] != ""){                       
                 ...
                 $htmlTabla .= "<tr ".$redlist_class.">
                                   ...
                                   if($usuario['estatus_upload'] == "0"){   
                                        $htmlTabla .="<td nowrap='nowrap' ><div id= 'boton_uploadFile' onclick='onAbrirDialog(\"".$usuario['id']."\",\"".$usuario['correo']."\" );' class=\"btnicon\" title=\"Upload Certificate\"><span><i  class=\"fa fa-upload\"> </i> Certificate</span></div></td>";
                                   }
