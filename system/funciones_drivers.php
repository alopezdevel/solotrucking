<?php 
  session_start();
  include("funciones_genericas.php"); // Generic functions lib
  ...
  function get_drivers(){
    ...
    $sql = "SELECT ct_operadores.iConsecutivo as id, 
            CASE eTipoLicencia
                WHEN '1' THEN 'Federal / B1'
                WHEN '2' THEN 'Commercial / CDL-1'
                END AS TipoLicencia, 
            ct_operadores.sNombre AS nombre, 
            DATE_FORMAT(ct_operadores.dFechaNacimiento,'%d %b %y') AS FechaNacimiento, 
            ct_operadores.iNumLicencia AS NumLicencia, 
            ct_operadores.iExperienciaYear AS Experiencia, 
            DATE_FORMAT(ct_operadores.dFechaExpiracionLicencia,'%d %b %y') AS FechaExpiracion, 
            ct_operadores.iNumLicencia, 
            DATE_FORMAT(ct_operadores.dFechaContratacion,'%d %b %y') AS FechaContratacion, 
            ct_entidad.sDescEntidad AS Entidad, ct_entidad.sCvePais AS Country  
            FROM  ct_operadores 
            LEFT JOIN   ct_entidad ON  ct_entidad.sCveEntidad = ct_operadores.iEntidad 
            LEFT JOIN cu_control_acceso ON cu_control_acceso.iConsecutivo = ct_operadores.iCompania 
            WHERE cu_control_acceso.sUsuario = '".$_SESSION['usuario_actual']."'".$filtroQuery;
  ...
  function save_driver(){
    ...
    if($_POST["edit_mode"] == 'true'){
      foreach($_POST as $campo => $valor){
        if($campo != "accion" and $campo != "edit_mode" and $campo != "iConsecutivo" and $campo != 'iCompania' and $campo != 'sLicenciaPDF' and $campo != 'sMVRPDF'){ //Estos campos no se insertan a la tabla
        ...
        $sql = "UPDATE ct_operadores SET ".implode(",",$valores)." WHERE iConsecutivo = '".$_POST['iConsecutivo']."'";
        $msj = '<p><span class="ui-icon ui-icon-circle-check" style="float:left; margin:0 7px 50px 0;"></span>Registro modificado exitosamente.</p>'; 
            
    }else{
        foreach($_POST as $campo => $valor){
           if($campo != "accion" and $campo != "edit_mode" and $campo != 'sLicenciaPDF' and $campo != 'sMVRPDF'){ //Estos campos no se insertan a la tabla
                array_push($campos ,$campo); 
                array_push($valores, $valor);
           }
       } 
        array_push($campos ,"iCompania");
        array_push($valores ,$iCompania);
        ...
        $msj = '<p><span class="ui-icon ui-icon-circle-check" style="float:left; margin:0 7px 50px 0;"></span>Registro agregado exitosamente.</p>';
  ...
  function load_driver(){
        ...
        $conexion->autocommit(FALSE);                                                                                                                                                                                                                                      
        $transaccion_exitosa = true;
        $sql = "SELECT iConsecutivo, sNombre, dFechaNacimiento, dFechaExpiracionLicencia, eTipoLicencia, iExperienciaYear, iNumLicencia, dFechaContratacion, iEntidad, iCompania FROM ct_operadores WHERE iConsecutivo = ".$id_driver;
        $result = $conexion->query($sql);
        $items = $result->num_rows;   
        if ($items > 0) {     
